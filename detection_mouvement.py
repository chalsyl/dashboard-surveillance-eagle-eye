...
CAMERAS_CONFIG = {
    "USB_CAM": {"source": 0},
    "PHONE_CAM": {"source": "http://192.168.1.6:4747/video"}, 
}

save_dir = os.path.expanduser("~/captures")
os.makedirs(save_dir, exist_ok=True)
try: os.chmod(save_dir, 0o777)
except: pass
...
def declencher_appel_asterisk(cam_name):
    if os.path.exists(LOCK_FILE):
        log(f"APPEL BLOQUE (Verrou actif)")
        return
    log(f"Initialisation appel Asterisk pour {cam_name}")
...
log("DEMARRAGE SYSTEME EAGLE EYE")
...
for name, conf in CAMERAS_CONFIG.items():
    ...
    if cap.isOpened():
        ...
        log(f"Camera {name} OK")
    else:
        log(f"Erreur Camera {name}")
...
try:
    while True:
        ...
        if system_on != prev_system_on:
           log("SYSTEME ENGAGE" if system_on else "MODE VEILLE")
        ...
        for name in CAMERAS_CONFIG.keys():
            ...
            try:
                cv2.imwrite(live_path, frame, [int(cv2.IMWRITE_JPEG_QUALITY), 80])
                            ...
                            if db:
                                ...
                                if call_on: 
                                    if (time.time() - last_phone_call_time) > 60:
                                        ...
                                        log("Appel bloque (Delai 1min)")
                        except: pass

        if not system_on: time.sleep(0.05)

except KeyboardInterrupt: log("ARRET MANUEL")
finally:
